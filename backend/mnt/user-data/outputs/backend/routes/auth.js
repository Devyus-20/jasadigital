...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    const [result] = await pool.query(
      "INSERT INTO users (nama, email, password, hp, role, aktif, created_at) VALUES (?, ?, ?, ?, 'pelanggan', 1, NOW())",
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!user.aktif)
      return res.status(403).json({ ok: false, msg: "Akun Anda dinonaktifkan. Hubungi admin." });
